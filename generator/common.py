...
class Session(object):
    def __init__(self, tx_port, rx_port, spec, mode, tx_pipelines, rx_pipelines):
        now = time.time()
    ...
    def update_port_stats(self, cli, now=None):
        ...
        self.__curr_stats = cli.bess.get_port_stats(self.__tx_port)
        if self.__rx_port != self.__tx_port:
            rx_stats = cli.bess.get_port_stats(self.__rx_port)
